	...
	<div>
		<div v-if="curIndex >= 0" class="quiz">
			...
			<div class="quiz__answers">
				<div class="quiz__answer" v-bind:class="{ 'quiz__answer--right': curQuestion.isAnswered && a.isRight, 'quiz__answer--wrong': curQuestion.isAnswered && !a.isRight }" v-for="(a, index) in curQuestion.answers">
		...
		<div v-if="curIndex == -1" style="text-align: center;">
			<div class="quiz__boxwrap">
				<h2 class="title is-3">Mach das Impf-Quiz!</h2>
				<p>Wie gut kennst du dich mit dem Thema Impfen aus?<br> Schaffst du es unsere Fragen zu meistern?</p><br>
				...
				<div class="has-margin-bottom-md"></div>
				<button class="button is-large is-success" v-on:click="startQuiz()">Start</button>
			</div>
		</div>
		...
		<div v-if="curIndex == -2" class="quiz__score">
			<div class="quiz__boxwrap">
				...
				<p v-if="percentageRight > 60">Auch wenn du schon ein echter Impfexperte zu sein scheinst, würden wir uns freuen wenn du dir auch noch unsere zahlreichen vertiefenden Informationsseiten anguckst</p>
				<p v-if="percentageRight >= 0 && percentageRight < 60">Wir hoffen du hast neues gelernt und würden uns freuen wenn du dir auch noch unsere zahlreichen vertiefenden Informationsseiten anguckst</p>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		answerQuestion(index) {
			if(this.curQuestion.isAnswered)
				return;
